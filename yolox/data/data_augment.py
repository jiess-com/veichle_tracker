...
def preproc(image, input_size, mean, std, swap=(2, 0, 1)):
    if len(image.shape) == 3: # 经过这里
        padded_img = np.ones((input_size[0], input_size[1], 3)) * 114.0
    else:
        padded_img = np.ones(input_size) * 114.0
    img = np.array(image)
    r = min(input_size[0] / img.shape[0], input_size[1] / img.shape[1]) # img.shape(2008, 3392) input_size(640,640)
    # print(r)
    ...
    if std is not None:
        padded_img /= std
    padded_img = padded_img.transpose(swap)# transpose 矩阵转置
